...
GCS_BUCKET="spotgourmand-uploads"
...
echo ""
echo "2/7 — Création du dépôt Artifact Registry..."
...
# 5. Créer le bucket GCS pour les images uploadées
echo ""
echo "5/7 — Création du bucket Google Cloud Storage..."

# Numéro de projet pour le service account par défaut de Compute/Cloud Run
PROJECT_NUMBER=$(gcloud projects describe $PROJECT_ID --format="value(projectNumber)")
COMPUTE_SA="${PROJECT_NUMBER}-compute@developer.gserviceaccount.com"

if gcloud storage buckets describe gs://$GCS_BUCKET &>/dev/null 2>&1; then
  echo "   Bucket '$GCS_BUCKET' existe déjà"
else
  gcloud storage buckets create gs://$GCS_BUCKET \
    --project=$PROJECT_ID \
    --location=$REGION \
    --quiet
  echo "   Bucket '$GCS_BUCKET' créé"
fi

# Lecture publique — les URLs https://storage.googleapis.com/... fonctionnent directement
gcloud storage buckets add-iam-policy-binding gs://$GCS_BUCKET \
  --member=allUsers \
  --role=roles/storage.objectViewer \
  --quiet
echo "   Lecture publique activée"

# Écriture pour le service account Cloud Run (pour uploader les images)
gcloud storage buckets add-iam-policy-binding gs://$GCS_BUCKET \
  --member=serviceAccount:${COMPUTE_SA} \
  --role=roles/storage.objectCreator \
  --quiet
echo "   Droits d'écriture accordés au service account Cloud Run"

# 6. Créer les services Cloud Run
echo ""
echo "6/7 — Création des services Cloud Run..."
...
echo ""
echo "  2. Configure le webhook Stripe dans le dashboard :"
echo "     URL : ${BACKEND_URL}/api/payments/webhook"
echo "     Événements : checkout.session.completed, checkout.session.expired"
echo "     → Copie le Signing secret dans STRIPE_WEBHOOK_SECRET"
echo ""
echo "  3. Lance : ./gcloud-deploy.sh"
echo ""
